...
@section('content')
    ...
    <div class="fixed bottom-4 left-1/2 transform -translate-x-1/2 space-y-2 sm:space-y-0 sm:space-x-2 sm:flex">
        <button onclick="handleView()"
            class="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-8 rounded-full border-2 border-black ml-4 flex items-center gap-2">
            Voir <img src="{{ asset('images/visualiser.svg') }}" alt="Voir" class="w-5 h-5">
        </button>

        <button onclick="handleDownload()"
            class="bg-green-500 hover:bg-green-600 text-white font-semibold py-2 px-4 rounded-full border-2 border-black ml-4 flex items-center gap-2">
            Télécharger<img src="{{ asset('images/telecharger.svg') }}" alt="Voir" class="w-8 h-8"></button>
        <button onclick="handleDelete()"
            class="bg-pink-500 hover:bg-pink-600 text-white font-semibold py-2 px-4 rounded-full border-2 border-black ml-4 flex items-center gap-2">
    ...
    <div id="modal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-white rounded-lg p-6 max-w-sm">
            <div class="text-center">
                <div id="modal-avatar"
                    class="w-64 h-64 mx-auto mb-4 overflow-hidden rounded-lg bg-gray-100 flex items-center justify-center">
    ...
    <script>
        ...
        <div onclick="selectAvatar('${avatar.avatar_id}', this)"
            ...
            <div class="mx-auto mb-3 bg-gray-100 rounded-lg flex items-center justify-center"
                 style="width:170px; height:190px;">
                <div style="width:150px; height:170px; display:flex; align-items:center; justify-content:center;">
                    ${avatar.avatar_svg}
                </div>
            </div>
